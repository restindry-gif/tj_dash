...
import { restoreCase, permanentDeleteCase } from '@/app/admin/cases/actions'
...
export function CaseRestoreButton({ caseId }: { caseId: string }) {
  ...
  if (restored) return <span className="text-xs text-emerald-400 font-medium">복구됨 ✓</span>
  if (purged) return <span className="text-xs text-slate-500">영구 삭제됨</span>

  const handleRestore = () => {
    startTransition(async () => {
      const result = await restoreCase(caseId)
  ...
  const handlePurge = () => {
    startTransition(async () => {
      const result = await permanentDeleteCase(caseId)
