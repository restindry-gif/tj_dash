...
import { ReportForm } from '@/app/staff/cases/[id]/report-form'
import { WorkSessionControl } from '@/app/staff/cases/[id]/work-session-control'
import { ReportItem } from '@/app/staff/cases/[id]/report-item'
...
interface RouteData { points: [number, number][]; startMeta?: PointMeta; endMeta?: PointMeta }
...
async function reverseGeocode(lat: number, lng: number): Promise<string> {
  try {
    ...
    return [addr.borough ?? addr.county, addr.suburb ?? addr.neighbourhood].filter(Boolean).join(' ') || '위치 정보 없음'
  } catch { return '위치 정보 없음' }
}
...
async function getRouteData(sessionId: string): Promise<RouteData> {
  ...
  return {
    points,
    startMeta: { time: formatDateTime(first.recorded_at, { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }), address: startAddr },
    endMeta: isMulti ? { time: formatDateTime(last.recorded_at, { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }), address: endAddr } : undefined,
  }
}

export default async function AdminFieldworkCasePage({
  params,
}: {
  ...
  const { data: client } = caseData?.client_id
    ? await supabase.from('profiles').select('full_name, phone').eq('id', caseData.client_id).maybeSingle()
    : { data: null }
  ...
  if (!caseData) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/admin/fieldwork" className="text-blue-400 hover:text-blue-300 text-sm mt-4 inline-block">← 목록으로</Link>
      </div>
    )
  }
  ...
  if (reports) {
    const routeReports = (reports as CaseReport[]).filter((r) => r.report_type === 'route' && r.session_id)
    ...
    <div className="space-y-4 max-w-2xl mx-auto">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold" style={{ color: '#e9c176' }}>드라이브 모드</p>
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-blue-500/15 border border-blue-500/20 flex items-center justify-center shrink-0">
                <span className="text-sm font-bold text-blue-400">{(client.full_name ?? '?')[0]}</span>
              </div>
              <div>
            ...
            {client.phone && (
              <a href={`tel:${client.phone}`} className="flex items-center gap-1.5 text-xs bg-green-500/10 text-green-400 border border-green-500/20 hover:bg-green-500/20 px-3 py-1.5 rounded-lg transition-colors">
